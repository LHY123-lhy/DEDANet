...
#Kappa
    pe = (sum_a1 * sum_a0 + (hist.sum() - sum_a1) * (hist.sum() - sum_a0)) / (hist.sum() ** 2)
    Kappa = (acc - pe) / (1 - pe)
    ...
    score_dict = {'acc': acc, 'miou': mean_iu, 'mf1':mean_F1, 'mrecall':mean_recall, 'mprecision':mean_precision,'mKappa':mean_Kappa}
