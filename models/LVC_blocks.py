...
class LVCBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_codes, channel_ratio=0.25, base_channel=64):
        ...
        self.conv_1 = ConvBlock(in_channels=2*in_channels, out_channels=2*in_channels, res_conv=True, stride=1)

        self.LVC = nn.Sequential(
            nn.Conv2d(2*in_channels, 2*in_channels, 1, bias=False),
            nn.BatchNorm2d(2*in_channels),
            nn.ReLU(inplace=True),
            Encoding(in_channels=2*in_channels, num_codes=num_codes),
            nn.BatchNorm1d(num_codes),
            nn.ReLU(inplace=True),
            Mean(dim=1))
        self.fc = nn.Sequential(nn.Linear(2*in_channels,2*in_channels), nn.Sigmoid())
        self.cnv1 = nn.Conv2d(2*in_channels, out_channels, kernel_size=1, stride=1, padding=0)

         # Stem stage: get the feature maps by conv block (copied form resnet.py) 进入conformer框架之前的处理
        self.conv1 = nn.Conv2d(2*in_channels, 2*in_channels, kernel_size=7, stride=1, padding=3, bias=False)  # 1 / 2 [112, 112]
