...
from models.networks import DEDANet
...
def val(args, val_loader, model, epoch):
    ...
    for iter, batched_inputs in enumerate(val_loader):
        ...
        #run the mdoel
        output,output1, output2, output3, output4 = model(pre_img_var, post_img_var)
        loss = BCEDiceLoss(output, target_var) + BCEDiceLoss(output1, target_var) + BCEDiceLoss(output2, target_var) + BCEDiceLoss(output3, target_var) + BCEDiceLoss(output4, target_var)
...
def trainValidateSegmentation(args):
    ...
    model = DEDANet(args, input_nc=3, output_nc=1)
    #model = Unet(input_nbr=3, label_nbr=1)
    ...
    if args.file_root == 'LEVIR':
        args.file_root = '/home/lihy/DATA/LEVIR-CD'
    elif args.file_root == 'BCDD':
        args.file_root = '/home/lihy/DATA/shujuji/BCDD256'
    elif args.file_root == 'SYSU':
        args.file_root = '/home/lihy/DATA/shujuji/SYSU-CD'
    elif args.file_root == 'CDD':
        args.file_root = '/home/lihy/DATA/shujuji/CDD'
    ...
    if os.path.isfile(logFileLoc):
        ...
        logger.write(
            "\n%s\t%s\t%s\t%s\t%s\t%s\t%s" % ('Epoch', 'Kappa (val)','OA (val)', 'IoU (val)', 'F1 (val)', 'R (val)', 'P (val)'))
    logger.flush()
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        torch.cuda.empty_cache()
        logger.write("\n%d\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f" % (epoch, score_val['Kappa'],score_val['OA'],score_val['IoU'],
    ...
    print("\nTest :\t Kappa (te) = %.4f\t OA (te) = %.4f\t IoU (te) = %.4f\t F1 (te) = %.4f\t R (te) = %.4f\t P (te) = %.4f" \
          % (score_test['Kappa'], score_test['OA'], score_test['IoU'], score_test['F1'], score_test['recall'], score_test['precision']))
    logger.write("\n%s\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f\t\t%.4f" % ('Test', score_test['Kappa'],score_test['OA'], score_test['IoU'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('--savedir', default='./test2', help='Directory to save the results')
